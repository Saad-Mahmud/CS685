...
if __name__ == "__main__":
    ...
    train_save_push(model, lr, epoch, tokenizer, train_dataset, test_dataset, ood_test_dataset, TMA[TM], output_dir, hh, max_len, bs)
    end_time = time.time()
    elapsed_time = end_time - start_time
    append_to_file("results.txt",f'{model_name} {DS} "training time: " {elapsed_time}')
    texts,A,Q = load_data_from_pickle(f'{DS}.pkl')
    del model
    del tokenizer
    tokenizer = AutoTokenizer.from_pretrained(model_name, torch_dtype=torch.bfloat16)
